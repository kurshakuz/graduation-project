/**
 * @file   nmpc_pc_learning_rover.cpp
...
void NMPC_PC::nmpc_init(struct acado_struct& acadostruct)
{
    ...
    for (int i = 0; i < (acadostruct.acado_N + 1); ++i)
    {
        for (int j = 0; j < acadostruct.acado_NOD; ++j)
            acadostruct.od[(i * acadostruct.acado_NOD) + j] = nmpc_inp_struct.init_params.at(j);
    }
                        ...
                        std::vector<double>& statesmeas)
{
    ...
    nan_check_for_estimates(params);

    // set the current state feedback
    set_measurements(acadostruct, params, statesmeas);
    ...
    for (int i = 0; i < acadostruct.acado_NU; ++i)
    {
        ...
        commandstruct.control_vel_vec[i] =
            (statesmeas.at(3) + nmpc_inp_struct.sampleTime * acadostruct.u[i]) / WHEEL_RADIUS_M;
...
void NMPC_PC::set_measurements(struct acado_struct& acadostruct,
                               ...
                               std::vector<double>& statesmeas)
{
    //    std::cout << "params = " << params.at(0) << ", " << params.at(1) << "\n";
    for (int i = 0; i < statesmeas.size(); i++)
        acadostruct.x0[i] = statesmeas.at(i);
    for (int i = 0; i < acadostruct.acado_N + 1; i++)
    {
        for (int j = 0; j < acadostruct.acado_NOD; j++)
            acadostruct.od[(i * acadostruct.acado_NOD) + j] = params.at(j);
    }
}
...
void NMPC_PC::nan_check_for_estimates(std::vector<double>& params)
{
    for (int i = 0; i < params.size(); i++)
    {
        if (std::isnan(params.at(i)) == true)
        {
            std::cerr << "NAN received for estimates! \n";
            std::cerr << "Initial param values are enforced! \n";
            params.clear();
            params.insert(params.end(), nmpc_inp_struct.init_params.begin(), nmpc_inp_struct.init_params.end());
            //            std::fill(params.begin(), params.end(), nmpc_inp_struct.init_params);
            continue;
        }
    }
}
